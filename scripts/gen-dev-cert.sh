...
# certificate. Browsers will show an untrusted-certificate warning for
# this pair -- that's expected, it's not signed by any real CA. Run from
# the repo root. See README.md's "HTTPS / TLS" section.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

out_dir="${TLS_CERT_DIR:-./certs}"
days="${1:-365}"
